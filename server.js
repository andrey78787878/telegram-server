  ...
  try {
    ...
    if (body.callback_query) {
      ...
      if (data.startsWith('in_progress_')) {
        ...
        await axios.post(`${TELEGRAM_API}/editMessageReplyMarkup`, {
          ...
          reply_markup: JSON.stringify(buildWorkButtons(id))
        });

        await sendMessage(chatId, `👤 Заявка #${id} принята в работу: @${user}`, null, Number(id));
      ...
      if (data.startsWith('delayed_') || data.startsWith('cancelled_')) {
        ...
        await sendMessage(chatId, `🔄 Заявка #${id}: ${status}`, null, Number(id));
        return res.sendStatus(200);
      }
      ...
      if (data.startsWith('executor_')) {
        ...
        await axios.post(`${TELEGRAM_API}/editMessageText`, {
          ...
          parse_mode: 'HTML',
          reply_markup: JSON.stringify(buildWorkButtons(pId))
        });
        return res.sendStatus(200);
      }
    }
    ...
    if (body.message && userState[body.message.chat.id]) {
      ...
      if (st.stage === 'photo' && body.message.photo) {
        ...
        const stream = (await axios.get(`${TELEGRAM_API.replace('bot', 'file/bot')}/${finfo.data.result.file_path}`, { responseType: 'stream' })).data;
    ...
    // ни одно условие не подошло — отправляем 200
    res.sendStatus(200);
  }
  catch (err) {
