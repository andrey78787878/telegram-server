...
app.post("/webhook", async (req, res) => {
  ...
  if (body.callback_query) {
    ...
    if (action === "accept") {
      await sendToGAS({ row, message_id, status: "В работе", username });
    ...
    if (action === "cancel") {
      await sendToGAS({ row, message_id, status: "Отменено", username });
      await editMessage(chat_id, message_id, `❌ Заявка отменена исполнителем: @${username}`);
    }

    if (action === "wait") {
      await sendToGAS({ row, message_id, status: "Ожидает поставки", username });
