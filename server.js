  ...
  try {
    ...
    if (body.callback_query) {
      ...
      if (data.startsWith('in_progress_')) {
        ...
        try {
          console.log('editMessageReplyMarkup params:', { chat_id: chatId, message_id: Number(id), reply_markup: buildWorkButtons(id) });
    ...
    if (body.message && userState[body.message.chat.id]) {
      ...
      if (st.stage==='photo' && body.message.photo) {
        ...
        const stream = (await axios.get(`${TELEGRAM_API}/${finfo.data.result.file_path}`,{responseType:'stream'})).data;
