...
app.post("/", async (req, res) => {
    ...
    if (callbackData === "Принято в работу") {
      ...
      if (rowIndex != null) {
        ...
        await sendTempMessage({
          chat_id,
          text: "Введите имя и/или компанию исполнителя:",
  ...
  if (body.message && body.message.photo) {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 1000)); // ⏳ Задержка для надёжности
      ...
      await sendTempMessage({
        chat_id,
        text: "Пожалуйста, добавьте комментарий по заявке (что было сделано):",
        reply_to_message_id: message_id
      });
    } catch (error) {
